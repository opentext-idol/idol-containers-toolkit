...
function getIndexerStatus() {
  index_status=$(wget -O- -nv -t5 --retry-connrefused --waitretry=10 "http://localhost:{{ (index .Values.contentPorts 0).container | int }}/a=indexergetstatus&index=$1" | sed "s/</\n</g" | grep status | cut -d ">" -f2)
}
...
function doDreinitial() {
  ...
  index_id = $(wget -q -nv -t 5 --retry-connrefused --waitretry=10 "http://localhost:{{ (index .Values.contentPorts 1).container | int }}/DRESYNC?" | grep INDEXID | cut -d "=" -f2)
  index_status=-7
...
if [[ ${is_primary} -eq 1 ]] 
then
  wget -Ogs.xml -nv -t 5 --retry-connrefused --waitretry=10 "http://localhost:{{ (index .Values.contentPorts 1).container | int }}/DRECREATEDBASE?DREDBNAME=Default"
  ...
  if getent hosts {{ .Values.dihName }}; then
    ...
    wget -Ogs.xml -nv -t 5 --retry-connrefused --waitretry=10 "http://{{ .Values.dihName }}:$dihaciport/a=getstatus"
    sed "s/</\n</g" gs.xml | grep "host" | grep "${hostname}\."
    if [ $? -eq 1 ]; then
      echo "[$(date)] $hostname not found in DIH, adding it." | tee -a $logfile
      wget -Odihadd.xml -nv -t 5 --retry-connrefused --waitretry=10 "http://{{ .Values.dihName }}:$dihaciport/a=enginemanagement&engineaction=add&host=$hostname&port=$port&disabled=true"
  ...
  if getent hosts {{ .Values.dahName }}; then
    ...
    if [ $? -eq 1 ]; then
      ...
      wget -O- -nv -t 5 --retry-connrefused --waitretry=10 "http://{{ .Values.dahName }}:$dahaciport/a=enginemanagement&engineaction=engineadd&enginehost=$hostname&engineport=$port"
